...
const generateContractNumber = async () => {
    ...
    if (lastContract.length > 0) {
        const lastNumber = parseInt(lastContract[0].contract_number.replace(prefix, ''));
...
const getAllContracts = async (req, res) => {
    try {
        ...
        if (search) {
            whereClause += ' AND (c.contract_number LIKE ? OR c.title LIKE ? OR cl.company_name LIKE ?)';
        ...
        if (req.user.role === 'locutor') {
            ...
            if (program[0].locutor_id !== req.user.id) {
                return res.status(403).json({
                    success: false,
                    message: 'Acesso negado. Você só pode visualizar contratos dos seus programas.'
                });
            }
        }
    ...
    try {
        ...
        const result = await transaction(async (connection) => {
            ...
            const [contractResult] = await connection.execute(
                ...
                [contract_number, client_id, program_id, ad_type_id, title, description,
                 start_date, end_date, total_spots, price_per_spot, total_value,
                 discount_percentage, final_value, req.user.id]
            );
...
const updateContract = async (req, res) => {
    try {
        ...
        const existingContracts = await query('SELECT * FROM contracts WHERE id = ?', [id]);
...
const deleteContract = async (req, res) => {
    try {
        ...
        // Verificar se contrato existe
        const existingContracts = await query('SELECT status FROM contracts WHERE id = ?', [id]);
        if (existingContracts.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'Contrato não encontrado'
        ...
        await transaction(async (connection) => {
            await connection.execute('DELETE FROM spot_schedule WHERE contract_id = ?', [id]);
            await connection.execute('DELETE FROM payments WHERE contract_id = ?', [id]);
            await connection.execute('DELETE FROM contract_files WHERE contract_id = ?', [id]);
...
const getContractStats = async (req, res) => {
    try {
        ...
        // Filtrar por role do usuário
        if (req.user.role === 'locutor') {
            whereClause = 'JOIN radio_programs rp ON c.program_id = rp.id WHERE rp.locutor_id = ?';
            params.push(req.user.id);
        }
        ...
        const monthlyContracts = await query(
            `SELECT 
             ...
             FROM contracts c ${whereClause}
             ${whereClause ? 'AND' : 'WHERE'} created_at >= DATE_SUB(NOW(), INTERVAL 12 MONTH)
