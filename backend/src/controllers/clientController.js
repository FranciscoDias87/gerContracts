...
const getAllClients = async (req, res) => {
  try {
    ...
    if (search) {
      whereClause +=
        " AND (company_name LIKE ? OR contact_name LIKE ? OR email LIKE ? OR cnpj LIKE ?)";
...
const createClient = async (req, res) => {
  try {
    ...
    if (cnpj) {
      ...
      existingClients = await query("SELECT id FROM clients WHERE email = ?", [
        email,
      ]);
    }
    ...
    const newClient = await query("SELECT * FROM clients WHERE id = ?", [
      result.insertId,
    ]);
...
const updateClient = async (req, res) => {
  try {
    ...
    const updatedClient = await query("SELECT * FROM clients WHERE id = ?", [
      id,
    ]);
  ...
  try {
    ...
    if (activeContracts.length > 0) {
      return res.status(400).json({
        success: false,
        message:
          "Não é possível deletar cliente com contratos ativos ou em rascunho",
      });
    }
