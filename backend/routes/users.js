...
const { validateUser, validateUserUpdate, validateId, handleValidationErrors } = require('../middleware/validation');
...
router.get('/locutors', authorize('admin', 'gerente'), userController.getLocutors);

// Rota para obter usuário por ID
router.get('/:id', validateId, authorize('admin', 'gerente'), userController.getUserById);
...
router.put('/:id', validateId, authorize('admin'), validateUserUpdate, userController.updateUser);
